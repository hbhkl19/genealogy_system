...
DECLARE fwd_visited INTEGER [] := ARRAY [p_start_id];
...
_fwd_idx INTEGER;
_rev_idx INTEGER;
BEGIN IF p_start_id = p_end_id THEN RETURN QUERY
SELECT ARRAY [p_start_id],
    ARRAY []::TEXT [];
RETURN;
END IF;
fwd_frontier := ARRAY [p_start_id];
rev_frontier := ARRAY [p_end_id];
WHILE fwd_depth + rev_depth < p_max_depth LOOP -- Expand forward frontier (one level)
IF array_length(fwd_frontier, 1) > 0 THEN fwd_depth := fwd_depth + 1;
DECLARE next_fwd INTEGER [] := ARRAY []::INTEGER [];
fwd_node INTEGER;
BEGIN FOREACH fwd_node IN ARRAY fwd_frontier LOOP FOR _neighbor IN
SELECT child_member_id AS to_id
FROM parent_child_relations
WHERE genealogy_id = p_genealogy_id
    AND parent_member_id = fwd_node
UNION ALL
SELECT parent_member_id
FROM parent_child_relations
WHERE genealogy_id = p_genealogy_id
    AND child_member_id = fwd_node
UNION ALL
SELECT spouse2_member_id
FROM marriages
WHERE genealogy_id = p_genealogy_id
    AND spouse1_member_id = fwd_node
UNION ALL
SELECT spouse1_member_id
FROM marriages
WHERE genealogy_id = p_genealogy_id
    AND spouse2_member_id = fwd_node LOOP IF NOT (_neighbor.to_id = ANY(fwd_visited)) THEN fwd_visited := fwd_visited || _neighbor.to_id;
next_fwd := next_fwd || _neighbor.to_id;
-- Check for meeting point
IF _neighbor.to_id = ANY(rev_visited) THEN meeting_id := _neighbor.to_id;
EXIT;
END IF;
END IF;
END LOOP;
IF meeting_id IS NOT NULL THEN EXIT;
END IF;
END LOOP;
...
IF array_length(rev_frontier, 1) > 0 THEN rev_depth := rev_depth + 1;
DECLARE next_rev INTEGER [] := ARRAY []::INTEGER [];
rev_node INTEGER;
BEGIN FOREACH rev_node IN ARRAY rev_frontier LOOP FOR _neighbor IN
SELECT child_member_id AS to_id
FROM parent_child_relations
WHERE genealogy_id = p_genealogy_id
    AND parent_member_id = rev_node
UNION ALL
SELECT parent_member_id
FROM parent_child_relations
WHERE genealogy_id = p_genealogy_id
    AND child_member_id = rev_node
UNION ALL
SELECT spouse2_member_id
FROM marriages
WHERE genealogy_id = p_genealogy_id
    AND spouse1_member_id = rev_node
UNION ALL
SELECT spouse1_member_id
FROM marriages
WHERE genealogy_id = p_genealogy_id
    AND spouse2_member_id = rev_node LOOP IF NOT (_neighbor.to_id = ANY(rev_visited)) THEN rev_visited := rev_visited || _neighbor.to_id;
next_rev := next_rev || _neighbor.to_id;
-- Check for meeting point
IF _neighbor.to_id = ANY(fwd_visited) THEN meeting_id := _neighbor.to_id;
EXIT;
END IF;
END IF;
END LOOP;
...
AND array_length(rev_frontier, 1) IS NULL THEN EXIT;
END IF;
END LOOP;
IF meeting_id IS NULL THEN RETURN;
END IF;
-- Path reconstruction: use recursive CTE to get path with relations
RETURN QUERY WITH RECURSIVE edges AS NOT MATERIALIZED (
    SELECT parent_member_id AS from_id,
        child_member_id AS to_id,
...
SET is_cycle USING cycle_path
SELECT fw.path || rw.path [2:array_length(rw.path, 1)],
    fw.rels || ARRAY(
        SELECT CASE
...
COMMENT ON FUNCTION find_relationship_path(INTEGER, INTEGER, INTEGER, INTEGER) IS 'Bidirectional BFS to find shortest relationship path between two members.
