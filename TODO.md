...
- `[x]` 已执行 `scripts/import_csv.py --input-dir data/generated --truncate`，PostgreSQL 导入成功；库内统计：10 个族谱、104000 成员、最大单族谱 50000、30 代、孤立成员 0。
- `[x]` 数据规则测试已覆盖同姓、跨族谱婚姻、子女随父姓、有子女者有配偶、母亲存在且来自外族谱。
- `[x]` 修正缩进树一代根节点渲染：无子女的一代节点不再保留透明展开按钮占位，展开其他分支后仍贴近最左侧显示。
- `[x]` 图形树按成员 ID 定位改为中心布局：目标成员居中，左侧逐代懒加载父母/祖先，右侧逐代懒加载子女/后代。
- `[x]` 图形树定位会清空旧画布，避免根节点模式和中心定位模式混在一起造成布局混乱。
- `[x]` 亲缘链路查询支持跨族谱搜索：路径图使用当前用户可访问的全部族谱关系边，允许通过跨族谱婚姻或父母关系找通路，同时避免展示不可访问族谱成员。
- `[x]` 亲缘链路查询优化为按 frontier 查邻居的双向 BFS，避免跨族谱时反复递归扫描全图关系边；真实 PostgreSQL 数据中跨族谱配偶查询约 10ms，大族谱远距离查询约 90ms。
- `[x]` 本轮验证：`pytest` 15 passed，`compileall app scripts migrations` 通过，`flask --app app routes` 通过。
